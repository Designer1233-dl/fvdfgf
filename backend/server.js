...
const ROOT_DEFAULTS = {
  ...
  xrocketPayApiToken: String(process.env.XROCKET_PAY_API_TOKEN || "").trim(),
  xrocketPayBaseUrl: String(
    process.env.XROCKET_PAY_BASE_URL ||
      (String(process.env.XROCKET_PAY_USE_TESTNET || "").trim() === "true"
        ? "https://pay.api.testnet.xrocket.exchange"
        : "https://pay.api.xrocket.exchange")
  )
    .trim()
    .replace(/\/+$/, ""),
  xrocketInvoiceExpiresIn: clampInt(process.env.XROCKET_INVOICE_EXPIRES_IN, 60, 2678400, 3600),
  xrocketUsername: String(process.env.XROCKET_USERNAME || "xrocket").trim().replace(/^@/, ""),
...
async function handleApi(req, res, url, body) {
  ...
  if (method === "GET" && pathname === "/api/health") {
    sendJson(res, 200, {
      ...
      config: {
        ...
        xrocketPayEnabled: Boolean(CONFIG.xrocketPayApiToken),
  ...
  if (method === "GET" && pathname === "/api/blogger/raffles") {
    const viewer = resolveTelegramViewer(req, body);
    const player = getOrCreatePlayer(viewer);
    assertBloggerAccess(player.telegramId);
    persistState();
    sendJson(res, 200, {
      bloggerAccess: serializeBloggerAccess(player.telegramId),
      raffles: getBloggerRaffles(player.telegramId),
    });
    return;
  }

  if (method === "POST" && pathname === "/api/profile/blogger/request") {
    const viewer = resolveTelegramViewer(req, body);
    const player = getOrCreatePlayer(viewer);
    const request = submitBloggerRequest(player, body);
    persistState();
    sendJson(res, 201, {
      profile: serializeProfile(player),
      request: serializeBloggerRequest(request),
    });
    return;
  }

  if (method === "POST" && pathname === "/api/blogger/raffles") {
    const viewer = resolveTelegramViewer(req, body);
    const player = getOrCreatePlayer(viewer);
    assertBloggerAccess(player.telegramId);
    const draft = await createPendingRaffleDraft(body, player);
  ...
  const bloggerVerifyRaffleMatch = pathname.match(/^\/api\/blogger\/raffles\/([^/]+)\/verify-payment$/);
  if (method === "POST" && bloggerVerifyRaffleMatch) {
    const viewer = resolveTelegramViewer(req, body);
    const player = getOrCreatePlayer(viewer);
    assertBloggerAccess(player.telegramId);
    const draft = state.pendingRafflePayments.find(
      (item) => item.id === bloggerVerifyRaffleMatch[1] && String(item.createdByTelegramId || "") === String(player.telegramId)
    );
    if (!draft) {
    ...
    persistState();
    sendJson(res, 200, {
      raffle: raffle ? serializeRaffle(raffle) : serializePendingRaffleDraft(draft),
  ...
  const bloggerApproveMatch = pathname.match(/^\/api\/admin\/bloggers\/requests\/([^/]+)\/approve$/);
  if (method === "POST" && bloggerApproveMatch) {
    assertAdmin(adminSession);
    const result = approveBloggerRequest(bloggerApproveMatch[1], adminSession);
    persistState();
    sendJson(res, 200, result);
    return;
  }

  const bloggerRejectMatch = pathname.match(/^\/api\/admin\/bloggers\/requests\/([^/]+)\/reject$/);
  if (method === "POST" && bloggerRejectMatch) {
    assertAdmin(adminSession);
    const result = rejectBloggerRequest(bloggerRejectMatch[1], adminSession);
    persistState();
    sendJson(res, 200, result);
    return;
  }

  const bloggerRevokeMatch = pathname.match(/^\/api\/admin\/bloggers\/requests\/([^/]+)\/revoke$/);
  if (method === "POST" && bloggerRevokeMatch) {
    assertAdmin(adminSession);
    const result = revokeBloggerAccess(bloggerRevokeMatch[1], adminSession);
    persistState();
    sendJson(res, 200, result);
    return;
  }
  ...
  if (method === "POST" && pathname === "/api/admin/jackpots") {
    assertAdmin(adminSession);
    const jackpot = createJackpotRoom(body, adminSession);
    state.jackpots.push(jackpot);
    persistState();
    sendJson(res, 201, {
      jackpot: serializeJackpot(jackpot, { admin: true }),
      shareLink: jackpot.shareLink,
    });
    return;
  }
  ...
  const adminFinalizeJackpotMatch = pathname.match(/^\/api\/admin\/jackpots\/([^/]+)\/finalize$/);
  if (method === "POST" && adminFinalizeJackpotMatch) {
    assertAdmin(adminSession);
    const jackpot = state.jackpots.find((item) => item.id === adminFinalizeJackpotMatch[1]);
    if (!jackpot) {
      sendJson(res, 404, { error: "jackpot_not_found" });
      return;
    }
    finalizeJackpot(jackpot);
    persistState();
    sendJson(res, 200, { jackpot: serializeJackpot(jackpot, { admin: true }) });
    return;
  }
  ...
  if (method === "GET" && pathname === "/api/jackpots") {
    const viewer = resolveOptionalTelegramUserFromHeader(req);
    const player = viewer ? getOrCreatePlayer(viewer) : null;
    if (viewer) {
      persistState();
    ...
    sendJson(res, 200, {
      jackpots: state.jackpots
        .slice()
        .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
        .map((jackpot) => serializeJackpot(jackpot, { player })),
    });
    return;
  }
  ...
  const publicJackpotMatch = pathname.match(/^\/api\/jackpots\/slug\/([^/]+)$/);
  if (method === "GET" && publicJackpotMatch) {
    const viewer = resolveOptionalTelegramUserFromHeader(req);
    const player = viewer ? getOrCreatePlayer(viewer) : null;
    const jackpot = state.jackpots.find((item) => item.slug === publicJackpotMatch[1]);
    if (!jackpot) {
      sendJson(res, 404, { error: "jackpot_not_found" });
      return;
    }
    ...
    sendJson(res, 200, { jackpot: serializeJackpot(jackpot, { player }) });
    return;
  }
  ...
  const joinJackpotMatch = pathname.match(/^\/api\/jackpots\/slug\/([^/]+)\/join$/);
  if (method === "POST" && joinJackpotMatch) {
    const jackpot = state.jackpots.find((item) => item.slug === joinJackpotMatch[1]);
    if (!jackpot) {
      sendJson(res, 404, { error: "jackpot_not_found" });
      return;
    }
    const viewer = resolveTelegramViewer(req, body);
    const participant = joinJackpot(jackpot, viewer);
    persistState();
    sendJson(res, 201, {
      jackpot: serializeJackpot(jackpot, { player: getOrCreatePlayer(viewer) }),
      participant,
    });
    return;
  }
...
function getAdminDashboardPayload() {
  const bloggerRequests = getAdminBloggerRequests();
  return {
    ...
    luckyDrops: state.pendingLuckyDropPayments
      ...
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)),
    jackpots: state.jackpots
      .map((jackpot) => serializeJackpot(jackpot, { admin: true }))
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)),
    cases: state.caseCatalog.map(serializeCase),
    ...
    settings: serializeWithdrawalSettings(),
    bloggerRequests,
    bloggers: bloggerRequests.filter((item) => item.status === "approved"),
  };
}

function getAdminBloggerRequests() {
  return state.bloggerRequests
    .slice()
    .sort((a, b) => new Date(b.requestedAt || b.createdAt || 0) - new Date(a.requestedAt || a.createdAt || 0))
    .map((request) => serializeBloggerRequest(request));
}

function getBloggerRequestForTelegramId(telegramId) {
  const normalizedTelegramId = String(telegramId || "").trim();
  if (!normalizedTelegramId) {
    return null;
  }
  return (
    state.bloggerRequests
      .slice()
      .sort((a, b) => new Date(b.requestedAt || b.createdAt || 0) - new Date(a.requestedAt || a.createdAt || 0))
      .find((request) => String(request.telegramId || "").trim() === normalizedTelegramId) || null
  );
}

function getBloggerAccessForTelegramId(telegramId) {
  const request = getBloggerRequestForTelegramId(telegramId);
  if (!request) {
    return {
      status: "none",
      isApproved: false,
...
function serializeBloggerAccess(telegramId) {
  return getBloggerAccessForTelegramId(telegramId);
}

function serializeBloggerRequest(request) {
  return {
    id: request.id,
    telegramId: request.telegramId,
    username: request.username,
    usernameLabel: request.username ? `@${request.username}` : request.displayName,
    ...
    requestedAt: request.requestedAt || "",
    reviewedAt: request.reviewedAt || "",
    reviewedByTelegramId: request.reviewedByTelegramId || "",
    reviewReason: request.reviewReason || "",
    isApproved: request.status === "approved",
...
function submitBloggerRequest(player, body) {
  const channelLink = String(body.channelLink || "").trim();
  const contactUsername = String(body.contactUsername || "").trim().replace(/^@/, "");
  if (!channelLink) {
    raise(400, "channel_link_required");
  }
  if (!contactUsername) {
    raise(400, "contact_required");
  }

  const now = new Date().toISOString();
  let request = state.bloggerRequests.find((item) => String(item.telegramId || "") === String(player.telegramId));
  if (!request) {
    request = {
    ...
    state.bloggerRequests.push(request);
    return request;
  }

  request.username = String(player.username || request.username || "");
  request.displayName = String(player.displayName || request.displayName || "Игрок");
  request.channelLink = channelLink;
  request.contactUsername = contactUsername;
  request.requestedAt = now;
  request.reviewedAt = "";
  request.reviewedByTelegramId = "";
  request.reviewReason = "";
  request.status = request.status === "approved" ? "approved" : "pending";
  return request;
}

function approveBloggerRequest(requestId, adminSession) {
  const request = state.bloggerRequests.find((item) => item.id === requestId);
  if (!request) {
    raise(404, "blogger_request_not_found");
  }
  request.status = "approved";
  request.reviewedAt = new Date().toISOString();
  request.reviewedByTelegramId = String(adminSession.telegramId || "");
  request.reviewReason = "";
  return {
    ok: true,
    request: serializeBloggerRequest(request),
...
function rejectBloggerRequest(requestId, adminSession) {
  const request = state.bloggerRequests.find((item) => item.id === requestId);
  if (!request) {
    raise(404, "blogger_request_not_found");
  }
  ...
  request.reviewedAt = new Date().toISOString();
  request.reviewedByTelegramId = String(adminSession.telegramId || "");
  request.reviewReason = "Отклонено админом";
  return {
    ok: true,
    request: serializeBloggerRequest(request),
  };
...
function revokeBloggerAccess(requestId, adminSession) {
  const request = state.bloggerRequests.find((item) => item.id === requestId);
  if (!request) {
    raise(404, "blogger_request_not_found");
  }
  ...
  request.reviewedAt = new Date().toISOString();
  request.reviewedByTelegramId = String(adminSession.telegramId || "");
  request.reviewReason = "Доступ снят админом";
  return {
    ok: true,
    request: serializeBloggerRequest(request),
  };
...
function assertBloggerAccess(telegramId) {
  if (!getBloggerAccessForTelegramId(telegramId).isApproved) {
    raise(403, "blogger_access_required");
  }
}

function getBloggerRaffles(telegramId) {
  const normalizedTelegramId = String(telegramId || "").trim();
  const drafts = state.pendingRafflePayments
    .filter((item) => String(item.createdByTelegramId || "") === normalizedTelegramId)
    .map((draft) => ({ kind: "draft", createdAt: draft.createdAt || "", raffle: serializePendingRaffleDraft(draft) }));
  const active = state.raffles
    .filter((item) => String(item.createdByTelegramId || "") === normalizedTelegramId)
    .map((raffle) => ({ kind: "raffle", createdAt: raffle.createdAt || "", raffle: serializeRaffle(raffle) }));
  return drafts
    .concat(active)
    .sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
    .map((item) => item.raffle);
}
...
async function createPendingRaffleDraft(body, adminSession) {
  ...
  return {
    ...
    payment: await createInvoicePayment({
      ...
      amount: prizeAmount,
      description: [title, prizeText].filter(Boolean).join(" • ").slice(0, 1024) || title.slice(0, 1024),
...
function createJackpotRoom(body, adminSession) {
  const title = String(body.title || "").trim() || "Джекпот-комната";
  const freeEntryAmount = clampNumber(body.freeEntryAmount, 0.01, 25000, NaN);
  const timerMinutes = clampInt(body.timerMinutes, 1, 60 * 24 * 30, 10);
  if (!Number.isFinite(freeEntryAmount)) {
    raise(400, "jackpot_free_entry_required");
  }
  ...
  return {
    ...
    slug,
    title,
    freeEntryAmount: roundMoney(freeEntryAmount),
    prizeAsset: CONFIG.cryptoPayAsset,
    timerMinutes,
    createdAt: new Date().toISOString(),
    activatedAt: new Date().toISOString(),
    ...
    finishedAt: "",
    status: "active",
    shareLink: buildShareLink({ type: "jackpot", slug }),
    participants: [],
    winner: null,
    resultText: "",
    createdByTelegramId: String(adminSession.telegramId || ""),
...
function joinJackpot(jackpot, viewer) {
  if (jackpot.status !== "active") {
    raise(409, "jackpot_closed");
  }

  const player = getOrCreatePlayer(viewer);
  const existing = jackpot.participants.find((item) => item.telegramId === player.telegramId);
  if (existing) {
    return serializeJackpotParticipant(existing, jackpot);
  }
  ...
  const participant = {
    ...
    photoUrl: player.photoUrl,
    joinedAt: new Date().toISOString(),
    entryAmount: roundMoney(jackpot.freeEntryAmount || 0),
  };
  jackpot.participants.push(participant);
  return serializeJackpotParticipant(participant, jackpot);
}
...
function finalizeRaffle(raffle) {
  ...
  raffle.resultText = winners.length
    ? winners
        ...
        .join(" | ")
    : "Победителей нет";
  raffle.finishedAt = new Date().toISOString();
  raffle.payoutDistributedAt = raffle.finishedAt;
  return raffle;
}

function finalizeJackpot(jackpot) {
  if (jackpot.status === "completed") {
    return jackpot;
  }
  if (jackpot.status !== "active") {
    raise(409, "jackpot_not_active");
  }

  jackpot.status = "completed";
  jackpot.finishedAt = new Date().toISOString();
  const pool = jackpot.participants.slice();
  if (!pool.length) {
    jackpot.winner = null;
    jackpot.resultText = "Участников не было";
    return jackpot;
  }

  const winnerIndex = Math.floor(Math.random() * pool.length);
  const chosen = pool[winnerIndex];
  const prizeAmount = roundMoney((jackpot.freeEntryAmount || 0) * pool.length);
  jackpot.winner = {
    participantId: chosen.id,
    telegramId: chosen.telegramId,
    username: chosen.username,
    ...
    photoUrl: chosen.photoUrl,
    prizeAmount,
    prizeAsset: jackpot.prizeAsset,
  };
  jackpot.resultText = `${chosen.username ? `@${chosen.username}` : chosen.displayName} забрал банк ${formatMoney(prizeAmount)} ${jackpot.prizeAsset}`;
  creditPlayerBalance(chosen, prizeAmount, {
    type: "jackpot_win",
    title: jackpot.title,
    asset: jackpot.prizeAsset,
    id: jackpot.id,
  });
  return jackpot;
}
...
async function requestWithdrawal(player, viewer, body) {
  ...
  if (state.withdrawalSettings.autoMode) {
    try {
      const issued = await issueWithdrawalPayout(player, viewer, roundedAmount, method);
      return issued;
    } catch (error) {
  ...
  player.transactions.unshift({
    ...
    amount: -roundedAmount,
    asset: CONFIG.cryptoPayAsset,
    note: `Заявка на ручной вывод через ${getProviderLabel(method)}`,
    createdAt,
  });
  player.transactions = player.transactions.slice(0, 60);
...
async function issueWithdrawalPayout(player, viewer, amount, method) {
  const payout = await createWithdrawalPayout(
    method,
    amount,
    Number(String(viewer.id || viewer.telegramId || "0")) || undefined
  );
  const createdAt = new Date().toISOString();
  ...
  const withdrawal = {
    ...
    asset: CONFIG.cryptoPayAsset,
    method,
    status: payout.isMock ? "mock_issued" : "issued",
    createdAt,
...
async function approveWithdrawalRequest(withdrawalId, adminSession) {
  ...
  const payout = await createWithdrawalPayout(withdrawal.method, withdrawal.amount, Number(player.telegramId) || undefined);

  withdrawal.status = payout.isMock ? "mock_issued" : "issued";
  withdrawal.reviewedAt = new Date().toISOString();
  withdrawal.reviewReason = "";
  withdrawal.checkId = payout.checkId;
  withdrawal.checkUrl = payout.checkUrl;
...
function autoFinalizeDueJackpots() {
  let changed = false;
  for (const jackpot of state.jackpots) {
    if (jackpot.status === "active" && jackpot.endsAt && new Date(jackpot.endsAt).getTime() <= Date.now()) {
      finalizeJackpot(jackpot);
      changed = true;
    }
  }
...
async function createInvoicePayment({ provider, key, amount, description, payload }) {
  const normalizedProvider = normalizePaymentProvider(provider) || "cryptobot";
  const now = new Date().toISOString();
  if (normalizedProvider === "xrocket") {
    if (!CONFIG.xrocketPayApiToken) {
      return buildMockPayment({
        provider: "xrocket",
        amount,
        createdAt: now,
        expiresInSeconds: CONFIG.xrocketInvoiceExpiresIn,
        link: `https://t.me/${CONFIG.xrocketUsername}`,
        invoiceId: `mock_${key}`,
      });
    }

    const invoice = await xrocketPayRequest("POST", "/api/v1/invoices", {
      amount: formatMoney(amount),
      asset: CONFIG.cryptoPayAsset,
      description,
      clientInvoiceId: key,
      hiddenMessage: description,
      expiredIn: CONFIG.xrocketInvoiceExpiresIn,
      payload: JSON.stringify(payload),
    });
    return normalizeXrocketInvoice(invoice, amount, now, key);
  }
  ...
  if (!CONFIG.cryptoPayApiToken) {
    return buildMockPayment({
      ...
      amount,
      createdAt: now,
      expiresInSeconds: CONFIG.cryptoPayInvoiceExpiresIn,
      link: `https://t.me/${CONFIG.cryptobotUsername}`,
      invoiceId: `mock_${key}`,
    });
  }
...
async function verifyPaymentObject(payment) {
  ...
  if (payment.isMock || payment.provider === "mock") {
    raise(409, "payment_verification_unavailable");
  }

  const provider = normalizePaymentProvider(payment.provider) || "cryptobot";
  if (provider === "xrocket") {
    const invoice = await getXrocketInvoice(payment.invoiceId);
    const normalized = normalizeXrocketInvoice(invoice, payment.amount, payment.createdAt, payment.invoiceId);
    payment.status = normalized.status;
    payment.botInvoiceUrl = normalized.botInvoiceUrl || payment.botInvoiceUrl || "";
    payment.miniAppInvoiceUrl = normalized.miniAppInvoiceUrl || payment.miniAppInvoiceUrl || "";
    payment.webAppInvoiceUrl = normalized.webAppInvoiceUrl || payment.webAppInvoiceUrl || "";
    payment.expiresAt = normalized.expiresAt || payment.expiresAt || "";
    if (normalized.status === "paid") {
      payment.paidAt = payment.paidAt || normalized.paidAt || new Date().toISOString();
      return payment;
    }
...
function buildMockPayment({ provider, amount, createdAt, expiresInSeconds, link, invoiceId }) {
  return {
    provider,
    status: "pending",
    amount: roundMoney(amount),
...
async function createWithdrawalPayout(method, amount, telegramUserId) {
  const normalizedMethod = normalizeWithdrawalMethod(method) || "cryptobot";
  if (normalizedMethod === "xrocket") {
    return createXrocketCheque(amount, telegramUserId);
  }
  return createCryptobotCheck(amount, telegramUserId);
}

async function createCryptobotCheck(amount, telegramUserId) {
  if (!CONFIG.cryptoPayApiToken) {
    return {
      checkId: `mock_${crypto.randomUUID()}`,
      checkUrl: `https://t.me/${CONFIG.cryptobotUsername}`,
      isMock: true,
    };
  }
  const check = await cryptoPayRequest("createCheck", {
    asset: CONFIG.cryptoPayAsset,
    amount: formatMoney(amount),
    pin_to_user_id: telegramUserId || undefined,
  });
  return {
...
async function createXrocketCheque(amount, telegramUserId) {
  if (!CONFIG.xrocketPayApiToken) {
    return {
      checkId: `mock_${crypto.randomUUID()}`,
      checkUrl: `https://t.me/${CONFIG.xrocketUsername}`,
      isMock: true,
    };
  }
  const check = await xrocketPayRequest("POST", "/api/v1/cheques", {
    amount: formatMoney(amount),
    asset: CONFIG.cryptoPayAsset,
    receiverTgUserId: telegramUserId || undefined,
    receiverType: telegramUserId ? "telegram_user_id" : undefined,
  });
  return {
    checkId: String(check.chequeId || check.checkId || check.id || ""),
    checkUrl: String(check.link || check.url || check.botUrl || `https://t.me/${CONFIG.xrocketUsername}`),
    isMock: false,
  };
}

async function getXrocketInvoice(invoiceId) {
  const normalizedId = String(invoiceId || "").trim();
  if (!normalizedId) {
    raise(404, "invoice_not_found");
  }
  const attempts = [
    () => xrocketPayRequest("GET", "/api/v1/invoice", { invoiceId: normalizedId }),
    () => xrocketPayRequest("GET", "/api/v1/invoices", { invoiceId: normalizedId }),
    () => xrocketPayRequest("GET", `/api/v1/invoices/${encodeURIComponent(normalizedId)}`),
  ];
  let lastError = null;
  ...
  throw lastError || Object.assign(new Error("xrocket_invoice_lookup_failed"), { statusCode: 502 });
}
...
function serializeJackpot(jackpot, options = {}) {
  const player = options.player || null;
  const participantCount = Array.isArray(jackpot.participants) ? jackpot.participants.length : 0;
  const prizePool = roundMoney((jackpot.freeEntryAmount || 0) * participantCount);
  const telegramId = String(player?.telegramId || "").trim();
  const joined = telegramId
    ? jackpot.participants.some((item) => String(item.telegramId || "") === telegramId)
    : false;
  return {
    id: jackpot.id,
    slug: jackpot.slug,
    title: jackpot.title,
    freeEntryAmount: roundMoney(jackpot.freeEntryAmount || 0),
    prizeAsset: jackpot.prizeAsset || CONFIG.cryptoPayAsset,
    timerMinutes: clampInt(jackpot.timerMinutes, 1, 60 * 24 * 30, 10),
    createdAt: jackpot.createdAt || "",
    activatedAt: jackpot.activatedAt || "",
    endsAt: jackpot.endsAt || "",
    finishedAt: jackpot.finishedAt || "",
    status: jackpot.status || "active",
    shareLink: jackpot.shareLink || buildShareLink({ type: "jackpot", slug: jackpot.slug || randomSlug() }),
    participantCount,
    prizePool,
    joined,
    winChancePercent: participantCount ? Number((100 / participantCount).toFixed(2)) : 0,
    participants: Array.isArray(jackpot.participants)
      ? jackpot.participants.map((participant) => serializeJackpotParticipant(participant, jackpot))
      : [],
    winner: jackpot.winner
      ? {
          ...jackpot.winner,
          usernameLabel: jackpot.winner.username ? `@${jackpot.winner.username}` : jackpot.winner.displayName,
        }
      : null,
    resultText: String(jackpot.resultText || ""),
  };
}
...
function serializeJackpotParticipant(participant, jackpot) {
  const participantCount = Math.max(1, Array.isArray(jackpot.participants) ? jackpot.participants.length : 1);
  return {
    ...participant,
    entryAmount: roundMoney(participant.entryAmount || jackpot.freeEntryAmount || 0),
    usernameLabel: participant.username ? `@${participant.username}` : participant.displayName,
    winChancePercent: Number((100 / participantCount).toFixed(2)),
  };
}
...
function serializePayment(payment) {
  ...
  return {
    provider: payment.provider || "cryptobot",
    ...
    isMock: Boolean(payment.isMock || payment.provider === "mock"),
...
function serializeWithdrawal(withdrawal) {
  return {
    ...
    providerLabel: getProviderLabel(withdrawal.method),
  };
}
...
function serializeProfile(player) {
  return {
    ...
    bloggerAccess: getBloggerAccessForTelegramId(player.telegramId),
  };
}
...
function buildShareLink({ slug, type = "raffle" }) {
  const startPrefix = type === "jackpot" ? "jackpot_" : "raffle_";
  const queryKey = type === "jackpot" ? "jackpot" : "raffle";
  if (CONFIG.botUsername && CONFIG.botUsername !== "your_bot") {
    return `https://t.me/${CONFIG.botUsername}?startapp=${encodeURIComponent(`${startPrefix}${slug}`)}`;
  }
  if (CONFIG.webAppUrl && !CONFIG.webAppUrl.includes("your-domain")) {
    const separator = CONFIG.webAppUrl.includes("?") ? "&" : "?";
    return `${CONFIG.webAppUrl}${separator}${queryKey}=${encodeURIComponent(slug)}`;
  }
  return `/?${queryKey}=${encodeURIComponent(slug)}`;
}
...
function normalizeState(parsed) {
  return {
    ...
    pendingLuckyDropPayments: Array.isArray(parsed.pendingLuckyDropPayments)
      ...
      : [],
    jackpots: Array.isArray(parsed.jackpots) ? parsed.jackpots.map(normalizeJackpot) : [],
    pendingDepositPayments: Array.isArray(parsed.pendingDepositPayments)
      ...
      : [],
    bloggerRequests: Array.isArray(parsed.bloggerRequests)
      ? parsed.bloggerRequests.map(normalizeBloggerRequest)
      : [],
...
function normalizePendingRaffleDraft(draft) {
  return {
    ...
    prizeText: String(draft.prizeText || "").trim(),
...
function normalizeBloggerRequest(request) {
  return {
    id: request.id || crypto.randomUUID(),
    telegramId: String(request.telegramId || "").trim(),
    username: String(request.username || "").trim(),
    displayName: String(request.displayName || "").trim() || "Игрок",
    channelLink: String(request.channelLink || "").trim(),
    contactUsername: String(request.contactUsername || "").trim().replace(/^@/, ""),
    status: String(request.status || "pending"),
    requestedAt: request.requestedAt || request.createdAt || new Date().toISOString(),
    reviewedAt: request.reviewedAt || "",
    reviewedByTelegramId: String(request.reviewedByTelegramId || "").trim(),
    reviewReason: String(request.reviewReason || "").trim(),
  };
}
...
function normalizeRaffle(raffle) {
  return {
    ...
    prizeText: String(raffle.prizeText || "").trim(),
    prizeAmount: roundMoney(raffle.prizeAmount || 0),
    ...
    activatedAt: raffle.activatedAt || "",
    endsAt: raffle.endsAt || "",
    finishedAt: raffle.finishedAt || raffle.payoutDistributedAt || "",
...
function normalizeJackpot(jackpot) {
  return {
    id: jackpot.id || crypto.randomUUID(),
    slug: jackpot.slug || randomSlug(),
    title: String(jackpot.title || "").trim() || "Джекпот-комната",
    freeEntryAmount: roundMoney(jackpot.freeEntryAmount || jackpot.entryAmount || 0),
    prizeAsset: String(jackpot.prizeAsset || CONFIG.cryptoPayAsset).trim().toUpperCase(),
    timerMinutes: clampInt(jackpot.timerMinutes, 1, 60 * 24 * 30, 10),
    createdAt: jackpot.createdAt || new Date().toISOString(),
    activatedAt: jackpot.activatedAt || jackpot.createdAt || new Date().toISOString(),
    endsAt: jackpot.endsAt || "",
    finishedAt: jackpot.finishedAt || "",
    status: String(jackpot.status || "active"),
    shareLink: jackpot.shareLink || buildShareLink({ type: "jackpot", slug: jackpot.slug || randomSlug() }),
    participants: Array.isArray(jackpot.participants) ? jackpot.participants : [],
    winner: jackpot.winner || null,
    resultText: String(jackpot.resultText || ""),
    createdByTelegramId: String(jackpot.createdByTelegramId || ""),
  };
}
...
async function xrocketPayRequest(method, endpoint, payload = undefined) {
  const requestUrl = new URL(endpoint, `${CONFIG.xrocketPayBaseUrl}/`);
    ...
    const request = https.request(
      requestUrl,
      {
        ...
        headers: {
          ...
          Authorization: `Bearer ${CONFIG.xrocketPayApiToken}`,
        ...
        response.on("end", () => {
          ...
          if (raw) {
            try {
              ...
              reject(Object.assign(new Error("xrocket_invalid_response"), { statusCode: 502 }));
              return;
            }
          }
          if (response.statusCode && response.statusCode >= 400) {
            reject(
              Object.assign(
                new Error(
                  parsed.detail || parsed.title || parsed.message || parsed.error || `xrocket_http_${response.statusCode}`
                ),
...
function normalizePaymentProvider(provider) {
  const normalized = String(provider || "cryptobot").trim().toLowerCase();
  if (normalized === "xrocket") {
    return "xrocket";
  }
  if (["cryptobot", "crypto_pay", "crypto-pay", "crypto pay", "mock"].includes(normalized)) {
    return "cryptobot";
  }
  return "";
}
...
function normalizeWithdrawalMethod(method) {
  const normalized = String(method || "cryptobot").trim().toLowerCase();
  return normalized === "cryptobot" || normalized === "xrocket" ? normalized : "";
}

function getProviderLabel(provider) {
  return normalizePaymentProvider(provider) === "xrocket" ? "xRocket" : "CryptoBot";
}

function normalizeXrocketInvoice(invoice, amount, createdAt, fallbackInvoiceId) {
  const status = normalizeXrocketInvoiceStatus(
    invoice.status || invoice.invoiceStatus || invoice.state || invoice.paymentStatus || "pending"
  );
  return {
    provider: "xrocket",
    status,
    amount: roundMoney(invoice.amount || amount || 0),
    asset: String(invoice.asset || CONFIG.cryptoPayAsset).trim().toUpperCase(),
    createdAt: invoice.createdAt || invoice.created || createdAt || new Date().toISOString(),
    expiresAt: String(invoice.expiredAt || invoice.expiresAt || invoice.expireAt || ""),
    invoiceId: String(invoice.invoiceId || invoice.id || invoice.invoiceID || fallbackInvoiceId || ""),
    botInvoiceUrl: String(invoice.botPayUrl || invoice.botInvoiceUrl || invoice.payUrl || invoice.url || ""),
    miniAppInvoiceUrl: String(invoice.miniAppPayUrl || invoice.miniAppInvoiceUrl || ""),
    webAppInvoiceUrl: String(invoice.webAppPayUrl || invoice.webAppInvoiceUrl || invoice.link || ""),
    paidAt: String(invoice.paidAt || invoice.completedAt || ""),
    isMock: false,
  };
}

function normalizeXrocketInvoiceStatus(status) {

...
const FINISHED_JACKPOT_REVEAL_MS = FINISHED_ROULETTE_SPIN_MS + FINISHED_ROULETTE_REVEAL_DELAY_MS + 400;
var luckyDropActive = [];
...
const state = {
  ...
  pendingBloggerDraftId: "",
  lastBloggerDraftPayment: null,
  lastBloggerDraftShareLink: "",
  lastWithdrawal: null,
  recentFinishedRaffle: null,
  recentFinishedJackpot: null,
};
...
const elements = {
  ...
  bloggerMenuToggle: document.getElementById("blogger-menu-toggle"),
  ...
  quickFormat: document.querySelector("#quick-info .quick-pill:nth-child(2) strong"),
  ...
  createJackpotForm: document.getElementById("create-jackpot-form"),
  ...
  adminJackpotList: document.getElementById("admin-jackpot-list"),
  adminJackpotHistoryList: document.getElementById("admin-jackpot-history-list"),
  ...
  bloggerAccessStatusCard: document.getElementById("blogger-access-status-card"),
  bloggerAccessTitle: document.getElementById("blogger-access-title"),
  bloggerAccessText: document.getElementById("blogger-access-text"),
  bloggerRequestForm: document.getElementById("blogger-request-form"),
  bloggerRequestMessage: document.getElementById("blogger-request-message"),
  openBloggerPanelButton: document.getElementById("open-blogger-panel-button"),
  bloggerSheet: document.getElementById("blogger-sheet"),
  bloggerSheetBackdrop: document.getElementById("blogger-sheet-backdrop"),
  closeBloggerButton: document.getElementById("close-blogger-button"),
  bloggerCreateRaffleForm: document.getElementById("blogger-create-raffle-form"),
  bloggerPaymentCard: document.getElementById("blogger-payment-card"),
  bloggerPaymentText: document.getElementById("blogger-payment-text"),
  bloggerPayButton: document.getElementById("blogger-pay-button"),
  bloggerVerifyButton: document.getElementById("blogger-verify-button"),
  bloggerShareCard: document.getElementById("blogger-share-card"),
  bloggerShareText: document.getElementById("blogger-share-text"),
  bloggerCopyLinkButton: document.getElementById("blogger-copy-link-button"),
  bloggerOpenLinkButton: document.getElementById("blogger-open-link-button"),
  bloggerRafflesList: document.getElementById("blogger-raffles-list"),
  bloggerMessage: document.getElementById("blogger-message"),
  adminBloggerRequestsList: document.getElementById("admin-blogger-requests-list"),
  adminBloggersList: document.getElementById("admin-bloggers-list"),
};
...
async function boot() {
  ...
  state.currentJackpotSlug = getJackpotSlugFromLocation();
...
function applyStaticTextFixes() {
  ...
  setText("landing-title", "От Илюшки");
  ...
  setText("blogger-menu-toggle", "Блогерский");
  ...
  setText("blogger-access-title", "Заявка на доступ");
  setText("blogger-access-text", "Заполни заявку, чтобы получить свой раздел для создания рулеток.");
  setText("open-blogger-panel-button", "Открыть блогерский");
  setText("close-blogger-button", "Закрыть");
  setText("blogger-pay-button", "Оплатить счет");
  setText("blogger-verify-button", "Проверить и запустить");
  setText("blogger-copy-link-button", "Скопировать");
  setText("blogger-open-link-button", "Открыть");
  ...
  setPlaceholder("blogger-channel-link-input", "https://t.me/...");
  setPlaceholder("blogger-contact-input", "@username");
  setPlaceholder("blogger-raffle-title-input", "Новый денежный розыгрыш");
  setPlaceholder("blogger-raffle-prize-text-input", "Можно оставить пустым");
...
function bindEvents() {
  ...
  elements.adminMenuToggle.addEventListener("click", () => {
    ...
    if (state.ui.adminPanelOpen) {
      state.ui.bloggerOpen = false;
    }
    state.ui.adminMenuOpen = false;
    renderAdminVisibility();
  });
  elements.bloggerMenuToggle?.addEventListener("click", () => {
    state.ui.adminMenuOpen = false;
    openBloggerSheet();
  });
  ...
  elements.bloggerSheetBackdrop?.addEventListener("click", closeBloggerSheet);
  elements.closeBloggerButton?.addEventListener("click", closeBloggerSheet);
  elements.profileHistoryToggle.addEventListener("click", () => {
    state.ui.historyOpen = !state.ui.historyOpen;
    renderProfileSheet();
  });
  elements.bloggerRequestForm?.addEventListener("submit", onSubmitBloggerRequest);
  elements.openBloggerPanelButton?.addEventListener("click", openBloggerSheet);
  elements.bloggerCreateRaffleForm?.addEventListener("submit", onCreateBloggerRaffle);
  elements.bloggerPayButton?.addEventListener("click", () => openExternal(elements.bloggerPayButton.dataset.link || ""));
  elements.bloggerVerifyButton?.addEventListener("click", verifyBloggerRafflePayment);
  elements.bloggerCopyLinkButton?.addEventListener("click", () => copyText(elements.bloggerShareText.textContent || ""));
  elements.bloggerOpenLinkButton?.addEventListener("click", () => openExternal(elements.bloggerOpenLinkButton.dataset.link || ""));
  ...
  elements.createJackpotForm?.addEventListener("submit", onCreateJackpot);
...
async function refreshAll() {
  ...
  const [health, rafflesPayload, jackpotsPayload, profilePayload, dashboardPayload, tasksPayload, luckyDropPayload, bloggerRafflesPayload] = await Promise.all(requests);
  state.health = health;
  ...
  state.jackpots = Array.isArray(jackpotsPayload?.jackpots) ? jackpotsPayload.jackpots : [];
  state.activeJackpots = state.jackpots.filter((item) => isActiveJackpot(item));
  state.recentFinishedRaffle = findRecentFinishedRaffle();
  state.recentFinishedJackpot = findRecentFinishedJackpot();
  ...
  state.bloggerRaffles = Array.isArray(bloggerRafflesPayload?.raffles) ? bloggerRafflesPayload.raffles : [];
  syncCurrentRaffle();
  render();
}
...
async function fetchJackpots() {
  try {
    return await api("/api/jackpots", { method: "GET" });
  } catch (error) {
    return { jackpots: [] };
  }
}
...
async function fetchBloggerRaffles() {
  try {
    return await api("/api/blogger/raffles", { method: "GET" });
  } catch (error) {
    return { raffles: [] };
  }
}
...
function syncCurrentRaffle() {
  const directJackpot = state.currentJackpotSlug
    ? state.jackpots.find((item) => item.slug === state.currentJackpotSlug) || null
    : null;
  const canShowDirectJackpot = directJackpot && (isActiveJackpot(directJackpot) || isFreshFinishedJackpot(directJackpot));
  if (directJackpot && isCompletedJackpot(directJackpot) && !isFreshFinishedJackpot(directJackpot)) {
    state.currentJackpotSlug = "";
    clearJackpotLocation();
  }
  if (canShowDirectJackpot) {
    state.currentJackpot = directJackpot;
    state.currentJackpotSlug = directJackpot.slug;
  ...
  if (state.currentRaffle) {
    ...
    state.currentJackpot = null;
  } else {
    const featuredJackpot = pickFeaturedJackpot();
    state.currentJackpot = featuredJackpot;
    state.currentJackpotSlug = featuredJackpot?.slug || "";
...
function renderAdminVisibility() {
  const canAccessAdmin = Boolean(state.health?.admin?.canAccess);
  const canAccessBlogger = Boolean(state.profile?.bloggerAccess?.isApproved);
  const authorized = Boolean(state.dashboard && state.adminToken);
  const hasMenu = canAccessAdmin || canAccessBlogger;
  elements.adminNav.classList.toggle("hidden", !hasMenu);
  elements.adminMenuPanel.classList.toggle("hidden", !hasMenu || !state.ui.adminMenuOpen);
  elements.adminMenuToggle.classList.toggle("hidden", !canAccessAdmin);
  elements.bloggerMenuToggle?.classList.toggle("hidden", !canAccessBlogger);
  elements.adminLoginCard.classList.toggle("hidden", !canAccessAdmin || !state.ui.adminPanelOpen || authorized);
  elements.adminPanel.classList.toggle("hidden", !canAccessAdmin || !state.ui.adminPanelOpen || !authorized);
}
...
function renderCurrentRaffle() {
  ...
  const jackpot = state.currentJackpot;

  if (jackpot) {
    elements.landingTitle.textContent = "\u041E\u0442 \u0418\u043B\u044E\u0448\u043A\u0438";
    elements.playerCard?.classList.remove("hidden");
    renderCurrentJackpot(jackpot);
    return;
  }
  elements.landingTitle.textContent = "От Илюшки";
  ...
  elements.landingTitle.textContent = "\u041E\u0442 \u0418\u043B\u044E\u0448\u043A\u0438";
  if (!raffle) {
    elements.landingSubtitle.textContent = "\u0421\u0435\u0439\u0447\u0430\u0441 \u0442\u0438\u0445\u043E";
    elements.heroCopy.textContent = "\u041A\u043E\u0433\u0434\u0430 \u043F\u043E\u044F\u0432\u0438\u0442\u0441\u044F \u043D\u043E\u0432\u0430\u044F \u0438\u0433\u0440\u0430, \u0434\u0436\u0435\u043A\u043F\u043E\u0442-\u043A\u043E\u043C\u043D\u0430\u0442\u0430 \u0438\u043B\u0438 \u0440\u043E\u0437\u044B\u0433\u0440\u044B\u0448, \u043E\u043D\u0438 \u0441\u0440\u0430\u0437\u0443 \u043F\u043E\u043A\u0430\u0436\u0443\u0442\u0441\u044F \u0437\u0434\u0435\u0441\u044C.";
  ...
  if (elements.quickFormat) {
    elements.quickFormat.textContent = "\u041A\u043B\u0430\u0441\u0441\u0438\u043A \u0440\u043E\u0437\u044B\u0433\u0440\u044B\u0448\u0430";
  }
...
function renderCurrentJackpot(jackpot) {
  if (isCompletedJackpot(jackpot)) {
    renderFinishedJackpot(jackpot);
    return;
  }

  const now = getNowMs();
  const endsAt = jackpot.endsAt ? new Date(jackpot.endsAt).getTime() : 0;
  const joined = hasJoinedJackpot(jackpot);
  const participantCount = Number(jackpot.participantCount || 0);
  const isClosing = Boolean(endsAt && endsAt <= now);

  elements.landingTitle.textContent = "\u041E\u0442 \u0418\u043B\u044E\u0448\u043A\u0438";
  elements.landingSubtitle.textContent = "\u0416\u0438\u0432\u0430\u044F \u0434\u0436\u0435\u043A\u043F\u043E\u0442-\u043A\u043E\u043C\u043D\u0430\u0442\u0430";
  elements.heroCopy.textContent = isClosing
    ? "\u0422\u0430\u0439\u043C\u0435\u0440 \u043D\u0430 \u043D\u0443\u043B\u0435. \u041A\u043E\u043C\u043D\u0430\u0442\u0430 \u0434\u043E\u043A\u0440\u0443\u0447\u0438\u0432\u0430\u0435\u0442 \u0444\u0438\u043D\u0430\u043B \u0438 \u0441\u0435\u0439\u0447\u0430\u0441 \u043E\u0442\u0434\u0430\u0441\u0442 \u0432\u0435\u0441\u044C \u0431\u0430\u043D\u043A \u043E\u0434\u043D\u043E\u043C\u0443 \u0438\u0437 \u0443\u0447\u0430\u0441\u0442\u043D\u0438\u043A\u043E\u0432."
    : "\u0417\u0430\u043B\u0435\u0442\u0430\u0439 \u043F\u043E \u0441\u0441\u044B\u043B\u043A\u0435, \u0437\u0430\u0431\u0438\u0440\u0430\u0439 \u0444\u0440\u0438-\u0441\u0442\u0430\u0432\u043A\u0443 \u043E\u0442 \u0430\u0434\u043C\u0438\u043D\u043A\u0438 \u0438 \u043B\u043E\u0432\u0438 \u0448\u0430\u043D\u0441 \u0443\u043D\u0435\u0441\u0442\u0438 \u0432\u0435\u0441\u044C \u0431\u0430\u043D\u043A.";
  elements.raffleTitle.textContent = jackpot.title || "\u0414\u0436\u0435\u043A\u043F\u043E\u0442-\u043A\u043E\u043C\u043D\u0430\u0442\u0430";
  elements.raffleSubtitle.textContent = getJackpotPrizeLine(jackpot);
  elements.playerBadge.textContent = `${participantCount} \u0432 \u0438\u0433\u0440\u0435`;
  elements.playerStatus.textContent = isClosing
    ? "\u0424\u0438\u043D\u0430\u043B \u0443\u0436\u0435 \u0437\u0430\u043F\u0443\u0441\u043A\u0430\u0435\u0442\u0441\u044F."
    : `\u0411\u0430\u043D\u043A \u0437\u0430\u043A\u0440\u043E\u0435\u0442\u0441\u044F \u0447\u0435\u0440\u0435\u0437 ${formatCountdown(Math.max(0, endsAt - now))}`;
  elements.quickStatus.textContent = isClosing
    ? "\u0424\u0438\u043D\u0430\u043B \u0434\u0436\u0435\u043A\u043F\u043E\u0442\u0430"
    : joined
      ? "\u0424\u0440\u0438-\u0441\u0442\u0430\u0432\u043A\u0430 \u0432 \u0431\u0430\u043D\u043A\u0435"
      : "\u041C\u043E\u0436\u043D\u043E \u0437\u0430\u0439\u0442\u0438";
  if (elements.quickFormat) {
    elements.quickFormat.textContent = "\u0416\u0438\u0432\u0430\u044F \u0434\u0436\u0435\u043A\u043F\u043E\u0442-\u043A\u043E\u043C\u043D\u0430\u0442\u0430";
  }
  elements.joinButton.classList.remove("hidden");
  elements.tasksButton?.classList.add("hidden");
  elements.emptyProfileButton.classList.add("hidden");
  elements.joinButton.textContent = joined
    ? "\u0422\u044B \u0443\u0436\u0435 \u0432 \u0431\u0430\u043D\u043A\u0435"
    : `\u0417\u0430\u0431\u0440\u0430\u0442\u044C ${formatMoney(jackpot.freeEntryAmount)} ${jackpot.prizeAsset || ""}`.trim();
  elements.joinButton.disabled = joined || isClosing;
  renderActiveJackpotVisual(jackpot);
}

function renderFinishedJackpot(jackpot) {
  const winner = getViewerJackpotWinner(jackpot);
  const hasWinner = Boolean(jackpot?.winner);
  const phase = getFinishedJackpotPhase(jackpot);

  elements.landingTitle.textContent = "\u041E\u0442 \u0418\u043B\u044E\u0448\u043A\u0438";
  elements.landingSubtitle.textContent = phase.mode === "final"
    ? "\u0411\u0430\u043D\u043A \u0440\u0430\u0437\u044B\u0433\u0440\u0430\u043D"
    : "\u0414\u043E\u043A\u0440\u0443\u0447\u0438\u0432\u0430\u0435\u043C \u0444\u0438\u043D\u0430\u043B";
  elements.heroCopy.textContent = !hasWinner
    ? "\u041A\u043E\u043C\u043D\u0430\u0442\u0430 \u0437\u0430\u043A\u0440\u044B\u043B\u0430\u0441\u044C, \u043D\u043E \u0432 \u043D\u0435\u0435 \u043D\u0438\u043A\u0442\u043E \u043D\u0435 \u0437\u0430\u0448\u0435\u043B."
    : phase.mode === "final"
      ? "\u041A\u043E\u043C\u043D\u0430\u0442\u0430 \u0437\u0430\u043A\u0440\u044B\u043B\u0430\u0441\u044C, \u0430 \u0432\u0435\u0441\u044C \u0431\u0430\u043D\u043A \u0443\u043B\u0435\u0442\u0435\u043B \u043E\u0434\u043D\u043E\u043C\u0443 \u0443\u0447\u0430\u0441\u0442\u043D\u0438\u043A\u0443."
      : "\u0421\u0435\u0439\u0447\u0430\u0441 \u0432\u0438\u0434\u043D\u043E, \u043A\u0430\u043A \u043B\u0435\u043D\u0442\u0430 \u0434\u043E\u043A\u0440\u0443\u0442\u0438\u043B\u0430\u0441\u044C \u0434\u043E \u043F\u043E\u0431\u0435\u0434\u0438\u0442\u0435\u043B\u044F.";
  elements.raffleTitle.textContent = hasWinner
    ? phase.mode === "final"
      ? "\u041F\u043E\u0431\u0435\u0434\u0438\u0442\u0435\u043B\u044C \u0434\u0436\u0435\u043A\u043F\u043E\u0442\u0430"
      : "\u041B\u0435\u043D\u0442\u0430 \u043E\u0441\u0442\u0430\u043D\u0430\u0432\u043B\u0438\u0432\u0430\u0435\u0442\u0441\u044F"
    : "\u0414\u0436\u0435\u043A\u043F\u043E\u0442 \u0437\u0430\u043A\u0440\u044B\u0442";
  elements.raffleSubtitle.textContent = hasWinner
    ? `\u0411\u0430\u043D\u043A: ${formatMoney(jackpot.winner?.prizeAmount || 0)} ${jackpot.winner?.prizeAsset || jackpot.prizeAsset || ""}`.trim()
    : getJackpotPrizeLine(jackpot);
  elements.playerBadge.textContent = hasWinner ? "\u0424\u0438\u043D\u0430\u043B" : "\u041F\u0443\u0441\u0442\u043E";
  elements.playerStatus.textContent = winner
    ? "\u0422\u044B \u0437\u0430\u0431\u0440\u0430\u043B \u0431\u0430\u043D\u043A. \u041F\u043E\u0437\u0434\u0440\u0430\u0432\u043B\u044F\u0435\u043C."
    : hasWinner
      ? phase.mode === "final"
        ? "\u0411\u0430\u043D\u043A \u0443\u0436\u0435 \u0443\u0448\u0435\u043B \u043F\u043E\u0431\u0435\u0434\u0438\u0442\u0435\u043B\u044E."
        : "\u041E\u043F\u0440\u0435\u0434\u0435\u043B\u044F\u0435\u043C, \u043D\u0430 \u043A\u043E\u043C \u043E\u0441\u0442\u0430\u043D\u0435\u0442\u0441\u044F \u043B\u0435\u043D\u0442\u0430."
      : "\u041A\u043E\u043C\u043D\u0430\u0442\u0430 \u0437\u0430\u043A\u043E\u043D\u0447\u0438\u043B\u0430\u0441\u044C \u0431\u0435\u0437 \u0443\u0447\u0430\u0441\u0442\u043D\u0438\u043A\u043E\u0432.";
  elements.quickStatus.textContent = winner
    ? "\u0422\u044B \u0437\u0430\u0431\u0440\u0430\u043B \u0432\u0435\u0441\u044C \u0431\u0430\u043D\u043A"
    : hasWinner
      ? phase.mode === "final"
        ? "\u0411\u0430\u043D\u043A \u0440\u0430\u0437\u0434\u0430\u043D"
      ...
      : "\u0418\u0433\u0440\u0430 \u043F\u0443\u0441\u0442\u0430";
  if (elements.quickFormat) {
    elements.quickFormat.textContent = phase.mode === "final"
      ? "\u0424\u0438\u043D\u0430\u043B \u0434\u0436\u0435\u043A\u043F\u043E\u0442\u0430"
      : "\u041B\u0435\u043D\u0442\u0430 \u0434\u0436\u0435\u043A\u043F\u043E\u0442\u0430";
  }
  ...
  elements.tasksButton?.classList.add("hidden");
  elements.emptyProfileButton.classList.add("hidden");
  renderFinishedJackpotVisual(jackpot, winner, phase);
}
...
function renderAdminDashboard() {
  ...
  if (!dashboard) {
    ...
    elements.adminJackpotList.innerHTML = "";
    elements.adminJackpotHistoryList.innerHTML = "";
    elements.adminBloggerRequestsList.innerHTML = "";
    elements.adminBloggersList.innerHTML = "";
    return;
  }
  ...
  const jackpots = Array.isArray(dashboard.jackpots) ? dashboard.jackpots : [];
  ...
  const jackpotActive = jackpots.filter((item) => isActiveJackpot(item));
  const jackpotHistory = jackpots.filter((item) => isCompletedJackpot(item));
  ...
  elements.adminJackpotList.innerHTML = jackpotActive.length
    ? jackpotActive.map(renderAdminJackpotCard).join("")
    : '<div class="mini-card mini-card--muted">Активных джекпотов пока нет.</div>';
  elements.adminJackpotHistoryList.innerHTML = jackpotHistory.length
    ? jackpotHistory.map(renderAdminJackpotHistoryCard).join("")
    : '<div class="mini-card mini-card--muted">Завершенных джекпотов пока нет.</div>';
  elements.adminBloggerRequestsList.innerHTML = Array.isArray(dashboard.bloggerRequests) && dashboard.bloggerRequests.length
    ? dashboard.bloggerRequests.map(renderBloggerRequestCard).join("")
    : '<div class="mini-card mini-card--muted">Заявок пока нет.</div>';
  elements.adminBloggersList.innerHTML = Array.isArray(dashboard.bloggers) && dashboard.bloggers.length
    ? dashboard.bloggers.map(renderApprovedBloggerCard).join("")
    : '<div class="mini-card mini-card--muted">Пока никто не одобрен.</div>';
}
...
function renderBloggerAccess(profile) {
  const access = profile?.bloggerAccess || { status: "none", isApproved: false };
  ...
  elements.bloggerAccessStatusCard.classList.toggle("hidden", !profile);
  elements.bloggerAccessTitle.textContent = isApproved
    ? "Доступ одобрен"
    : isPending
      ? "Заявка на проверке"
      : isRejected
        ...
        : isRevoked
          ? "Доступ снят"
          : "Заявка на доступ";
  elements.bloggerAccessText.textContent = isApproved
    ? "Твой блогерский раздел открыт. Можно создавать рулетки."
    : isPending
      ? `Заявка отправлена${access.channelLink ? `: ${access.channelLink}` : ""}.`
      : isRejected
        ? "Можно отправить новую заявку."
        : isRevoked
          ? "Доступ снят администратором. Можно отправить новую заявку."
          : "Заполни заявку, чтобы получить свой раздел для создания рулеток.";
  elements.bloggerRequestForm.classList.toggle("hidden", isApproved || isPending);
  elements.openBloggerPanelButton.classList.toggle("hidden", !isApproved);
}

function renderBloggerSheet() {
  const profile = state.profile;
  const access = profile?.bloggerAccess || { isApproved: false };
  const opened = Boolean(state.ui.bloggerOpen && access.isApproved);
  elements.bloggerSheet.classList.toggle("hidden", !opened);
  elements.bloggerSheetBackdrop.classList.toggle("hidden", !opened);
  if (!opened) {
    return;
  }

  renderBloggerPaymentCard(state.lastBloggerDraftPayment);
  renderBloggerShareCard(state.lastBloggerDraftShareLink || "");
  elements.bloggerRafflesList.innerHTML = Array.isArray(state.bloggerRaffles) && state.bloggerRaffles.length
    ? state.bloggerRaffles.map(renderBloggerRaffleCard).join("")
    : '<div class="mini-card mini-card--muted">Пока нет созданных рулеток.</div>';
  elements.bloggerMessage.textContent = "";
}
...
function renderPaymentCard(payment) {
  ...
  elements.paymentCardText.textContent =
    `Счет в ${payment.providerLabel || formatProviderLabel(payment.provider)} создан на ${formatMoney(payment.amount)} ${payment.asset}. После оплаты нажми "Проверить и запустить".`;
  elements.payInvoiceButton.dataset.link = pickPaymentLink(payment);
  elements.verifyPaymentButton.disabled = !state.pendingDraftId;
}

function renderBloggerPaymentCard(payment) {
  if (!payment) {
    elements.bloggerPaymentCard.classList.add("hidden");
    return;
  }
  elements.bloggerPaymentCard.classList.remove("hidden");
  elements.bloggerPaymentText.textContent =
    `Счет в ${payment.providerLabel || formatProviderLabel(payment.provider)} создан на ${formatMoney(payment.amount)} ${payment.asset}.`;
  elements.bloggerPayButton.dataset.link = pickPaymentLink(payment);
  elements.bloggerVerifyButton.disabled = !state.pendingBloggerDraftId;
}

function renderBloggerShareCard(link) {
  if (!link) {
    elements.bloggerShareCard.classList.add("hidden");
    return;
  }
  elements.bloggerShareCard.classList.remove("hidden");
  elements.bloggerShareText.textContent = link;
  elements.bloggerOpenLinkButton.dataset.link = link;
}

function renderBloggerRequestCard(request) {
  const actions = [];
  if (request.status === "pending" || request.status === "rejected") {
    actions.push(`<button class="secondary small" type="button" data-action="approve-blogger-request" data-id="${escapeAttribute(request.id)}">Одобрить</button>`);
  }
  if (request.status === "pending") {
    actions.push(`<button class="secondary small" type="button" data-action="reject-blogger-request" data-id="${escapeAttribute(request.id)}">Отклонить</button>`);
  }
  return `
    <div class="mini-card">
      <strong>${escapeHtml(request.usernameLabel || request.displayName || "Блогер")}</strong>
      <span>${escapeHtml(request.channelLink || "")}</span>
      <span>${escapeHtml(request.contactUsername || "")}</span>
...
function renderApprovedBloggerCard(request) {
  const isExpanded = state.ui.adminBloggerDetailsId === request.id;
  const detailsButtonLabel = isExpanded ? "Скрыть" : "Подробнее";
  const detailsMarkup = isExpanded
    ? `
      <span>ID: ${escapeHtml(request.telegramId || "—")}</span>
      <span>Username: ${escapeHtml(request.username ? `@${request.username}` : "—")}</span>
      <span>Контакт: ${escapeHtml(request.contactUsername ? `@${request.contactUsername}` : "—")}</span>
      <span>Подал заявку: ${escapeHtml(formatDateTime(request.requestedAt) || "—")}</span>
      <span>Одобрен: ${escapeHtml(formatDateTime(request.reviewedAt) || "—")}</span>
    `
    : "";
  return `
    <div class="mini-card">
      ...
      <div class="inline-actions">
        <button class="secondary small" type="button" data-action="toggle-blogger-details" data-id="${escapeAttribute(request.id)}">${detailsButtonLabel}</button>
        <button class="secondary small" type="button" data-action="revoke-blogger-access" data-id="${escapeAttribute(request.id)}">Снять доступ</button>
      </div>
    </div>
  `;
}

function renderBloggerRaffleCard(raffle) {
  const actions = [];
  if (raffle.status === "pending_payment") {
    actions.push(`<button class="secondary small" type="button" data-action="blogger-verify-payment" data-id="${escapeAttribute(raffle.id)}">Проверить оплату</button>`);
  }
  if (raffle.status === "active") {
    actions.push(`<button class="secondary small" type="button" data-action="copy-link" data-link="${escapeAttribute(raffle.shareLink || "")}">Скопировать ссылку</button>`);
  }
  return `
    <div class="mini-card">
      <strong>${escapeHtml(raffle.title || "Рулетка")}</strong>
      <span>${escapeHtml(getRafflePrizeLine(raffle))}</span>
      <span>${formatStatus(raffle.status)}</span>
      ${raffle.shareLink ? `<a class="link-button" href="${escapeAttribute(raffle.shareLink)}" target="_blank" rel="noreferrer">Открыть</a>` : ""}
      <div class="inline-actions">${actions.join("")}</div>
    </div>
  `;
}

function openBloggerSheet() {
  if (!ensureBloggerAccess(elements.bloggerMessage)) {
    return;
  }
  state.ui.bloggerOpen = true;
  renderBloggerSheet();
}

function closeBloggerSheet() {
  state.ui.bloggerOpen = false;
  renderBloggerSheet();
}
...
function renderWithdrawalCard(withdrawal) {
  ...
  return `
    <div class="mini-card">
      ...
      <span>${escapeHtml(withdrawal.providerLabel || formatProviderLabel(withdrawal.method))}</span>
...
function renderAdminJackpotCard(jackpot) {
  return `
    <div class="mini-card">
      <strong>${escapeHtml(jackpot.title || "Джекпот-комната")}</strong>
      <span>${escapeHtml(getJackpotPrizeLine(jackpot))}</span>
      <span>${Number(jackpot.participantCount || 0)} участников • ${formatStatus(jackpot.status)}</span>
      <div class="inline-actions">
        <button class="secondary small" type="button" data-action="copy-link" data-link="${escapeAttribute(jackpot.shareLink || "")}">Скопировать ссылку</button>
        <button class="secondary small" type="button" data-action="finalize-jackpot" data-id="${escapeAttribute(jackpot.id)}">Завершить сейчас</button>
      </div>
    </div>
  `;
}

function renderAdminJackpotHistoryCard(jackpot) {
  const winner = jackpot?.winner?.usernameLabel || jackpot?.winner?.displayName || "Без победителя";
  return `
    <div class="mini-card">
      <strong>${escapeHtml(jackpot.title || "Джекпот-комната")}</strong>
      <span>${escapeHtml(getJackpotPrizeLine(jackpot))}</span>
...
function renderActiveJackpotVisual(jackpot) {
  const participants = Array.isArray(jackpot.participants) ? jackpot.participants : [];
  const source = participants.length ? participants.slice(0, 8) : createGhostParticipants();
  const orbitItems = source
    .map((item, index) => {
      const joined = state.profile?.telegramId && String(item.telegramId || "") === String(state.profile.telegramId);
      return `
        <div class="jackpot-orbit-card${joined ? " jackpot-orbit-card--viewer" : ""}" style="--orbit-index:${index}; --orbit-total:${Math.max(1, source.length)};">
          ${renderAvatarMarkup(item, "roulette-avatar")}
          <strong>${escapeHtml(item.usernameLabel || item.displayName || "Игрок")}</strong>
          <span>${joined ? "\u0422\u044B \u0432 \u0438\u0433\u0440\u0435" : `+${formatMoney(item.entryAmount || jackpot.freeEntryAmount || 0)} ${escapeHtml(jackpot.prizeAsset || "")}`}</span>
        </div>
      `;
    })
    .join("");
  ...
  setRouletteMarkup({
    key: `jackpot-active:${jackpot.id}:${participants.map((item) => item.id || item.telegramId || item.username || "").join("|")}`,
    html: `
      <div class="jackpot-stage">
        <div class="jackpot-core">
          <span class="jackpot-core__label">БАНК ДЖЕКПОТА</span>
          <strong>${escapeHtml(getJackpotPrizeLine(jackpot))}</strong>
          <span>${Number(jackpot.participantCount || 0)} участников внутри</span>
        </div>
        <div class="jackpot-orbit">${orbitItems}</div>
      </div>
    `,
    className: "roulette-stack roulette-stack--jackpot-stage",
    windowExpanded: false,
  });
}

function renderFinishedJackpotVisual(jackpot, viewerWinner, phase) {
  const winner = jackpot?.winner || null;
  if (winner && phase.mode !== "final") {
    setRouletteMarkup({
      key: `jackpot-spin:${jackpot.id}:${phase.mode}`,
      html: buildSequentialJackpotRoulette(jackpot),
      className: "roulette-stack",
      windowExpanded: false,
      onAfterRender: phase.mode === "sequence"
        ? (stack) => {
            applyRouletteStopOffset(stack);
            void stack.offsetWidth;
            stack.classList.add("roulette-stack--stopping");
  ...
  const audience = Array.isArray(jackpot.participants) ? jackpot.participants : [];
  const audienceMarkup = audience.length
    ? audience
        .slice(0, 12)
        .map((item) => {
          const isWinner = winner && isSamePerson(item, winner);
          const isViewer = viewerWinner && isSamePerson(item, viewerWinner);
          return `
            <div class="jackpot-audience-card${isWinner ? " jackpot-audience-card--winner" : ""}${isViewer ? " jackpot-audience-card--viewer" : ""}">
              ${renderAvatarMarkup(item, "roulette-avatar")}
              <div class="roulette-meta">
                <strong>${escapeHtml(item.usernameLabel || item.displayName || "Игрок")}</strong>
                <span>${isWinner ? "\u0417\u0430\u0431\u0440\u0430\u043B \u0431\u0430\u043D\u043A" : "\u0411\u044B\u043B \u0432 \u043A\u043E\u043C\u043D\u0430\u0442\u0435"}</span>
              </div>
            </div>
          `;
        })
  ...
  setRouletteMarkup({
    key: `jackpot-finished:${jackpot.id}:${winner?.telegramId || "none"}`,
    html: `
      <div class="jackpot-finish">
        <div class="jackpot-finish__halo"></div>
        <div class="jackpot-winner-card${viewerWinner ? " jackpot-winner-card--viewer" : ""}">
          ${winner ? renderAvatarMarkup(winner, "jackpot-winner-avatar") : '<div class="jackpot-winner-avatar">?</div>'}
          <div class="jackpot-winner-copy">
            <span>${winner ? "\u041F\u043E\u0431\u0435\u0434\u0438\u0442\u0435\u043B\u044C \u0434\u0436\u0435\u043A\u043F\u043E\u0442\u0430" : "\u041A\u043E\u043C\u043D\u0430\u0442\u0430 \u043F\u0443\u0441\u0442\u0430"}</span>
            <strong>${escapeHtml(winner?.usernameLabel || winner?.displayName || jackpot.resultText || "Без победителя")}</strong>
            <em>${escapeHtml(winner ? `${formatMoney(winner.prizeAmount || 0)} ${winner.prizeAsset || jackpot.prizeAsset || ""}` : getJackpotPrizeLine(jackpot))}</em>
          </div>
        </div>
        <div class="jackpot-audience">${audienceMarkup}</div>
      </div>
    `,
    className: "roulette-stack roulette-stack--jackpot-finished",
    windowExpanded: true,
  });
}
...
function buildSequentialJackpotRoulette(jackpot) {
  const participants = Array.isArray(jackpot.participants) && jackpot.participants.length
    ...
    .map((item) => {
      ...
      return `
        <div class="${classes}" data-stop-target="${item._stopTarget ? "true" : "false"}">
          ...
          <div class="roulette-meta">
            <strong>${escapeHtml(item.usernameLabel || item.displayName || "Игрок")}</strong>
            <span>${item._stopTarget ? "Победный слот" : isViewer ? "Это ты" : "Участник джекпота"}</span>
          </div>
        </div>
      `;
    })
...
function getFinishedJackpotPhase(jackpot) {
  if (!jackpot?.winner) {
    return {
      mode: "final",
      visualKey: `jackpot-final:${jackpot?.id || "empty"}:0`,
    };
  }

  const finishedAt = getJackpotFinishedAtMs(jackpot);
  const elapsed = Math.max(0, getNowMs() - finishedAt);
  if (elapsed < FINISHED_ROULETTE_SPIN_MS) {
    return {
      mode: "sequence",
      visualKey: `jackpot-sequence:${jackpot.id}`,
    };
  }
  if (elapsed < FINISHED_ROULETTE_SPIN_MS + FINISHED_ROULETTE_REVEAL_DELAY_MS) {
    return {
      mode: "hold",
      visualKey: `jackpot-hold:${jackpot.id}`,
    };
  }
  return {
    mode: "final",
    visualKey: `jackpot-final:${jackpot.id}`,
  };
}
...
function buildProfileHistory(profile) {
  ...
  for (const withdrawal of profile.recentWithdrawals || []) {
    rows.push({
      ...
      title: `Вывод через ${withdrawal.providerLabel || formatProviderLabel(withdrawal.method)}: ${formatStatus(withdrawal.status)}`,
      subtitle: withdrawal.checkUrl
        ? `Чек готов в ${withdrawal.providerLabel || formatProviderLabel(withdrawal.method)}`
        : `Чек в ${withdrawal.providerLabel || formatProviderLabel(withdrawal.method)} еще не готов`,
...
async function onAdminLogin(event) {
  ...
  const formElement = getEventForm(event);
  ...
  try {
    const payload = await api("/api/admin/raffles", {
      ...
      body: {
        ...
        paymentProvider: String(form.get("paymentProvider") || "cryptobot"),
      },
    });
...
async function onCreateJackpot(event) {
  event.preventDefault();
  const formElement = getEventForm(event);
  const form = new FormData(formElement);
  try {
    const payload = await api("/api/admin/jackpots", {
      method: "POST",
      admin: true,
      body: {
        title: String(form.get("title") || ""),
        freeEntryAmount: Number(form.get("freeEntryAmount") || 0),
        timerMinutes: Number(form.get("timerMinutes") || 1),
      },
    });
    resetFormElement(formElement);
    if (elements.createJackpotForm?.elements?.namedItem("timerMinutes")) {
      elements.createJackpotForm.elements.namedItem("timerMinutes").value = "5";
    }
    state.currentJackpotSlug = payload.jackpot?.slug || "";
    state.currentRaffleSlug = "";
    if (payload.shareLink) {
      setJackpotLocation(payload.jackpot?.slug || "");
      copyText(payload.shareLink);
    }
    await refreshAll();
    showMessage(elements.adminMessage, "\u0414\u0436\u0435\u043A\u043F\u043E\u0442-\u043A\u043E\u043C\u043D\u0430\u0442\u0430 \u0441\u043E\u0437\u0434\u0430\u043D\u0430. \u0421\u0441\u044B\u043B\u043A\u0430 \u0443\u0436\u0435 \u0433\u043E\u0442\u043E\u0432\u0430.", "win");
  } catch (error) {
    showMessage(elements.adminMessage, formatError(error.message), "error");
  }
}
...
async function finalizeJackpotNow(jackpotId) {
  try {
    await api(`/api/admin/jackpots/${encodeURIComponent(jackpotId)}/finalize`, {
      method: "POST",
      admin: true,
    });
    await refreshAll();
    showMessage(elements.adminMessage, "\u0414\u0436\u0435\u043A\u043F\u043E\u0442-\u043A\u043E\u043C\u043D\u0430\u0442\u0430 \u0437\u0430\u043A\u0440\u044B\u0442\u0430.", "win");
  } catch (error) {
    showMessage(elements.adminMessage, formatError(error.message), "error");
  }
}

async function onSubmitBloggerRequest(event) {
  event.preventDefault();
  const formElement = getEventForm(event);
  const form = new FormData(formElement);
  if (!ensureTelegramProfile(elements.bloggerRequestMessage)) {
    return;
  }
  try {
    const payload = await api("/api/profile/blogger/request", {
      method: "POST",
      body: {
        channelLink: String(form.get("channelLink") || ""),
        contactUsername: String(form.get("contactUsername") || ""),
      },
    });
    state.profile = payload.profile || state.profile;
    resetFormElement(formElement);
    await refreshAll();
    showMessage(elements.bloggerRequestMessage, "Заявка отправлена на проверку.", "win");
    render();
  } catch (error) {
    showMessage(elements.bloggerRequestMessage, formatPlayerError(error.message), "error");
  }
}

async function onCreateBloggerRaffle(event) {
  event.preventDefault();
  const formElement = getEventForm(event);
  const form = new FormData(formElement);
  if (!ensureBloggerAccess(elements.bloggerMessage)) {
  ...
  try {
    const payload = await api("/api/blogger/raffles", {
      method: "POST",
      body: {
        title: String(form.get("title") || ""),
        prizeText: String(form.get("prizeText") || ""),
    ...
    state.pendingBloggerDraftId = payload.raffle?.id || "";
    state.lastBloggerDraftPayment = payload.payment || null;
    state.lastBloggerDraftShareLink = "";
    renderBloggerPaymentCard(payload.payment);
    renderBloggerShareCard("");
    resetFormElement(formElement);
    await refreshAll();
    showMessage(elements.bloggerMessage, "Счет для блогерской рулетки создан. После оплаты нажми проверку.", "win");
  } catch (error) {
    showMessage(elements.bloggerMessage, formatPlayerError(error.message), "error");
  }
}

async function verifyBloggerRafflePayment(draftId = "") {
  const raffleId = String(draftId || state.pendingBloggerDraftId || "").trim();
  if (!raffleId) {
    showMessage(elements.bloggerMessage, "Сейчас нет счета для проверки.", "error");
    return;
  }
  try {
    const payload = await api(`/api/blogger/raffles/${encodeURIComponent(raffleId)}/verify-payment`, {
      method: "POST",
      body: {},
    });
    state.lastBloggerDraftPayment = payload.payment || null;
    renderBloggerPaymentCard(payload.payment);
    if (payload.shareLink) {
      state.pendingBloggerDraftId = "";
      state.lastBloggerDraftShareLink = payload.shareLink;
      renderBloggerShareCard(payload.shareLink);
      await refreshAll();
      showMessage(elements.bloggerMessage, "Оплата найдена. Рулетка запущена.", "win");
      return;
    }
    showMessage(elements.bloggerMessage, "Оплата еще не подтверждена. Попробуй снова позже.", "muted");
  } catch (error) {
    showMessage(elements.bloggerMessage, formatPlayerError(error.message), "error");
  }
}

async function approveBloggerRequestAction(requestId) {
  try {
    await api(`/api/admin/bloggers/requests/${encodeURIComponent(requestId)}/approve`, {
      method: "POST",
      admin: true,
    });
    await refreshAll();
    showMessage(elements.adminMessage, "Заявка блогера одобрена.", "win");
  } catch (error) {
    showMessage(elements.adminMessage, formatError(error.message), "error");
  }
}

async function rejectBloggerRequestAction(requestId) {
  try {
    await api(`/api/admin/bloggers/requests/${encodeURIComponent(requestId)}/reject`, {
      method: "POST",
      admin: true,
    });
    await refreshAll();
    showMessage(elements.adminMessage, "Заявка блогера отклонена.", "win");
  } catch (error) {
    showMessage(elements.adminMessage, formatError(error.message), "error");
  }
}

async function revokeBloggerAccessAction(requestId) {
  try {
    await api(`/api/admin/bloggers/requests/${encodeURIComponent(requestId)}/revoke`, {
      method: "POST",
      admin: true,
    });
    if (state.ui.adminBloggerDetailsId === requestId) {
      state.ui.adminBloggerDetailsId = "";
    }
    await refreshAll();
    showMessage(elements.adminMessage, "Блогерский доступ снят.", "win");
  } catch (error) {
    showMessage(elements.adminMessage, formatError(error.message), "error");
  }
}
...
async function joinCurrentRaffle() {
  if (state.currentJackpotSlug && state.currentJackpot) {
    await joinCurrentJackpot();
    return;
  }
...
async function joinCurrentJackpot() {
  if (!state.currentJackpotSlug) {
    showMessage(elements.playerMessage, "\u0414\u0436\u0435\u043A\u043F\u043E\u0442-\u043A\u043E\u043C\u043D\u0430\u0442\u0430 \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D\u0430.", "error");
    return;
  }
  if (!ensureTelegramProfile(elements.playerMessage)) {
    return;
  ...
  try {
    await api(`/api/jackpots/slug/${encodeURIComponent(state.currentJackpotSlug)}/join`, {
      method: "POST",
    });
    await refreshAll();
    showMessage(elements.playerMessage, "\u0424\u0440\u0438-\u0441\u0442\u0430\u0432\u043A\u0430 \u0434\u043E\u0431\u0430\u0432\u043B\u0435\u043D\u0430 \u0432 \u0431\u0430\u043D\u043A \u0434\u0436\u0435\u043A\u043F\u043E\u0442\u0430.", "win");
  } catch (error) {
    showMessage(elements.playerMessage, formatPlayerError(error.message), "error");
  }
}
...
function onDocumentClick(event) {
  ...
  if (action === "finalize-jackpot") {
    finalizeJackpotNow(button.dataset.id || "");
  }
  ...
  if (action === "approve-blogger-request") {
    approveBloggerRequestAction(button.dataset.id || "");
  }
  if (action === "reject-blogger-request") {
    rejectBloggerRequestAction(button.dataset.id || "");
  }
  if (action === "toggle-blogger-details") {
    const requestId = button.dataset.id || "";
    state.ui.adminBloggerDetailsId = state.ui.adminBloggerDetailsId === requestId ? "" : requestId;
    render();
  }
  if (action === "revoke-blogger-access") {
    revokeBloggerAccessAction(button.dataset.id || "");
  }
  if (action === "blogger-verify-payment") {
    state.pendingBloggerDraftId = button.dataset.id || "";
    verifyBloggerRafflePayment(button.dataset.id || "");
  }
}
...
function getJackpotSlugFromLocation() {
  const search = new URLSearchParams(window.location.search);
  const fromQuery = String(search.get("jackpot") || "").trim();
  if (fromQuery) {
    return fromQuery;
  }
  const startParam = String(state.auth.startParam || "").trim();
  if (startParam.startsWith("jackpot_")) {
    return startParam.slice("jackpot_".length);
  }
  return "";
}

function setJackpotLocation(slug) {
  if (!window?.history?.replaceState) {
    return;
  }
  const url = new URL(window.location.href);
  if (slug) {
    url.searchParams.set("jackpot", slug);
  } else {
    url.searchParams.delete("jackpot");
  }
  window.history.replaceState({}, "", `${url.pathname}${url.search}${url.hash}`);
}

function clearJackpotLocation() {
  setJackpotLocation("");
}
...
function pickFeaturedJackpot() {
  return state.activeJackpots[0] || null;
}
...
function isActiveJackpot(jackpot) {
  return Boolean(jackpot && jackpot.status === "active");
}

function isCompletedJackpot(jackpot) {
  return Boolean(jackpot && jackpot.status === "completed");
}
...
function getJackpotFinishedAtMs(jackpot) {
  return new Date(jackpot?.finishedAt || jackpot?.updatedAt || jackpot?.endsAt || 0).getTime();
}
...
function getJackpotPrizeLine(jackpot) {
  const total = Number(jackpot?.prizePool || Number(jackpot?.freeEntryAmount || 0) * Number(jackpot?.participantCount || 0) || 0);
  return `Банк ${formatMoney(total)} ${jackpot?.prizeAsset || ""} • фри-ставка ${formatMoney(jackpot?.freeEntryAmount || 0)} ${jackpot?.prizeAsset || ""}`.trim();
}
...
function hasJoinedJackpot(jackpot) {
  return Boolean(
    state.profile?.telegramId &&
      Array.isArray(jackpot?.participants) &&
      jackpot.participants.some((item) => String(item.telegramId || "") === String(state.profile.telegramId))
  );
}
...
function getViewerJackpotWinner(jackpot) {
  const telegramId = String(state.profile?.telegramId || state.auth.telegramId || "");
  if (!telegramId || !jackpot?.winner) {
    return null;
  }
  return String(jackpot.winner.telegramId || "") === telegramId ? jackpot.winner : null;
}
...
function findRecentFinishedJackpot() {
  const candidates = state.jackpots.filter((item) => isFreshFinishedJackpot(item));
  return candidates[0] || null;
}
...
function isFreshFinishedJackpot(jackpot) {
  if (!isCompletedJackpot(jackpot)) {
    return false;
  }
  const finishedAt = getJackpotFinishedAtMs(jackpot);
  ...
  return getNowMs() - finishedAt < FINISHED_JACKPOT_REVEAL_MS;
}
...
function buildWithdrawalText(withdrawal) {
  ...
  const providerLabel = withdrawal.providerLabel || formatProviderLabel(withdrawal.method);
  if (withdrawal.checkUrl) {
    return `Чек ${providerLabel} уже готов: ${withdrawal.amountLabel}`;
  }
  return `Заявка на вывод через ${providerLabel} создана: ${formatStatus(withdrawal.status)}`;
}
...
function ensureBloggerAccess(messageEl) {
  if (!ensureTelegramProfile(messageEl)) {
    return false;
  }
  if (state.profile?.bloggerAccess?.isApproved) {
    return true;
  }
  showMessage(messageEl, "Сначала дождитесь одобрения блогерского доступа.", "error");
  return false;
}
...
function formatProviderLabel(provider) {
  ...
  return normalized === "xrocket" ? "xRocket" : "CryptoBot";
}

function formatTransactionType(type) {
  if (type === "jackpot_win") {
    return "Выигрыш в джекпоте";
  }
  const map = {
  ...
  map.jackpot_win = "Выигрыш в джекпоте";
  return map[type] || beautifyCode(type);
}

function formatError(code) {
  if (code === "jackpot_not_found") {
    return "Джекпот-комната не найдена.";
  }
  if (code === "jackpot_not_active") {
    return "Джекпот-комната сейчас не активна.";
  }
  if (code === "jackpot_closed") {
    return "Джекпот-комната уже закрыта.";
  }
  if (code === "jackpot_free_entry_required") {
    return "Укажите сумму фри-ставки для джекпота.";
  }
  if (code === "blogger_access_required") {
    return "Сначала нужно одобрение блогерского доступа.";
  }
  const map = {
    ...
    channel_link_required: "Укажите ссылку на канал.",
    contact_required: "Укажите контакт для связи.",
    blogger_request_not_found: "Заявка блогера не найдена.",
    ...
    withdraw_method_invalid: "Выберите корректную площадку вывода.",
    payment_verification_unavailable: "Проверка оплаты недоступна, пока API выбранной площадки не подключен.",
    invoice_not_paid: "Инвойс еще не оплачен.",
    invoice_expired: "Срок действия инвойса истек.",
    xrocket_invalid_response: "xRocket вернул непонятный ответ. Проверьте токен и параметры API.",
    xrocket_invoice_lookup_failed: "Не удалось проверить инвойс xRocket.",
    not_found: "Нужная страница не найдена.",
    internal_error: "Что-то пошло не так на сервере.",
  };
  map.jackpot_not_found = "Джекпот-комната не найдена.";
  map.jackpot_not_active = "Джекпот-комната сейчас не активна.";
  map.jackpot_closed = "Джекпот-комната уже закрыта.";
  map.jackpot_free_entry_required = "Укажите сумму фри-ставки для джекпота.";
  return map[code] || map[String(code || "").toLowerCase()] || beautifyCode(code);
}

function formatPlayerError(code) {
  if (String(code || "").startsWith("jackpot_")) {
    return formatError(code);
  }
  const map = {
    blogger_access_required: "Сначала дождись одобрения блогерского доступа.",
    blogger_request_not_found: "Заявка не найдена.",
    channel_link_required: "Укажи ссылку на свой канал.",
    contact_required: "Укажи контакт для связи.",

...
function renderPublicRaffles() {
  if (elements.publicSection.classList.contains("hidden")) {
    return;
  }
    ...
    .map(
      (raffle) => `
        <button class="public-card${state.currentRaffle?.slug === raffle.slug ? " active" : ""}" type="button" data-slug="${raffle.slug}">
          <strong>${escapeHtml(raffle.title)}</strong>
          <small>${raffle.participantCount} участников</small>
        </button>
      `
    )
    .join("");
  ...
  for (const button of elements.publicRaffles.querySelectorAll("[data-slug]")) {
    button.addEventListener("click", async () => {
      ...
      history.replaceState({}, "", buildRaffleUrl(slug));
...
function renderCurrentRaffle() {
  ...
  elements.playerCard.classList.toggle("landing", !explicitRaffle);
  elements.playerBadge.classList.toggle("hidden", !explicitRaffle);
  ...
  elements.playerStatus.innerHTML = explicitRaffle
    ...
    : "";
  elements.rouletteTrack.parentElement.parentElement.classList.toggle("hidden", !explicitRaffle);

...
function bindEvents() {
  ...
  elements.profileHistoryToggle.addEventListener("click", toggleProfileHistory);
...
function renderProfileSheet() {
  ...
  elements.profileHistoryCard.classList.toggle("hidden", !state.profileUi.historyOpen);
  elements.profileHistoryToggle.textContent = state.profileUi.historyOpen ? "Скрыть историю" : "История";
...
function renderProfileHistory() {
  const withdrawals = Array.isArray(state.profile?.recentWithdrawals) ? state.profile.recentWithdrawals : [];
  elements.profileHistoryList.innerHTML = withdrawals.length
    ? withdrawals.map(renderWithdrawalHistoryItem).join("")
    : `<div class="list-card empty">История выводов пока пустая</div>`;
}

function renderWithdrawalHistoryItem(withdrawal) {
  const amount = Number(withdrawal.amount || 0);
  const asset = String(withdrawal.asset || state.config.payoutAsset || "USDT");
  const method = String(withdrawal.method || "cryptobot");
  const createdAt = formatHistoryDate(withdrawal.createdAt);
  const status = translateWithdrawalStatus(withdrawal.status);

  return `
    <article class="list-card history withdrawal-history-card">
      <div class="list-row">
        <strong>${escapeHtml(`${formatMoney(amount)} ${asset}`)}</strong>
        <span class="status-pill active">${escapeHtml(status)}</span>
      </div>
      <span>${escapeHtml(createdAt)} • ${escapeHtml(method === "cryptobot" ? "CryptoBot" : method)}</span>
      ${
        withdrawal.checkUrl
          ? `<div class="list-actions"><a class="secondary" target="_blank" rel="noreferrer" href="${escapeHtml(withdrawal.checkUrl)}">Забрать</a></div>`
          : ""
      }
    </article>
  `;
...
function toggleProfileHistory() {
  state.profileUi.historyOpen = !state.profileUi.historyOpen;
  renderProfileSheet();
}
...
function formatAdminError(message) {
  switch (message) {
    ...
    case "prize_amount_too_small":
      return "Сумма приза слишком маленькая для такого количества победителей";
    case "invoice_not_paid":
      return "Счет еще не оплачен";
    case "invoice_expired":
      return "Счет истек, создай новый";
    case "payment_verification_unavailable":
      return "Проверка оплаты недоступна, пока не настроен Crypto Pay";
...
function translateWithdrawalStatus(status) {

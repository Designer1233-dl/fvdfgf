...
function renderProfileVisibility() {
  ...
  if (state.auth.photoUrl) {
    elements.profileAvatar.innerHTML = `<img src="${escapeHtml(state.auth.photoUrl)}" alt="${escapeHtml(state.auth.displayName)}" />`;
  } else {
    elements.profileAvatar.textContent = initialsOf(state.auth.displayName);
  }
}
...
function renderProfileSheet() {
  ...
  if (state.auth.photoUrl) {
    elements.profileSheetAvatar.innerHTML = `<img src="${escapeHtml(state.auth.photoUrl)}" alt="${escapeHtml(name)}" />`;
  } else {
    elements.profileSheetAvatar.textContent = initialsOf(name);
  }
...
function renderRoulette(raffle, animateToWinner) {
  ...
  const baseTrack = participants.map((participant) => ({
    ...participant,
    isWinner: Array.isArray(raffle.winners)
      ? raffle.winners.some((winner) => winner.participantId === participant.id)
      : false,
  }));
  ...
  if (!animateToWinner || !Array.isArray(raffle.winners) || !raffle.winners.length) {
  ...
  elements.rouletteTrack.style.transform = "translateX(-34px)";
...
function startRouletteLoop(raffleKey, participantCount) {
  ...
  const tick = (now) => {
    ...
    previousTime = now;
    state.roulette.offset = (state.roulette.offset + delta * 0.05) % cycleWidth;

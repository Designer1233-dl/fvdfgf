...
function initTelegram() {
  ...
  state.auth.displayName = [user.first_name, user.last_name].filter(Boolean).join(" ").trim() || state.auth.username || "Игрок";
...
async function refreshAll() {
  ...
  syncCurrentRaffle();
  renderAdminVisibility(health.admin?.authorized || false);
  renderPublicRaffles();
  renderCurrentRaffle();
  if (state.adminToken) {
...
function syncCurrentRaffle() {
  const fromSlug = state.raffles.find((item) => item.slug === state.currentRaffleSlug);
  state.currentRaffle = fromSlug || state.raffles.find((item) => item.status === "active") || state.raffles[0] || null;
}

function renderAdminVisibility(isAuthorizedFromServer) {
  const isAuthorized = Boolean(state.adminToken && isAuthorizedFromServer);
  elements.adminLoginCard.classList.toggle("hidden", isAuthorized);
  elements.adminPanel.classList.toggle("hidden", !isAuthorized);
}
            ...
            (raffle) => `
              <article class="list-card">
                <div>
                  ...
                  <span>${escapeHtml(raffle.prizeText)} | Участников: ${raffle.participantCount} | Шанс: ${raffle.chancePercent}%</span>
                </div>
                <button class="secondary" type="button" data-finalize-id="${raffle.id}">Подвести итог</button>
              </article>
            `
          )
          .join("")
      : `<div class="list-card empty">Нет активных розыгрышей</div>`;

    elements.adminHistoryList.innerHTML = history.length
      ? history
          .map((raffle) => {
            const winners = raffle.winners.length
              ? raffle.winners.map((winner) => `Победитель ${winner.place}: ${escapeHtml(winner.usernameLabel)}`).join("<br />")
              : "Победителей нет";
            return `
...
function renderPublicRaffles() {
  if (!state.raffles.length) {
    elements.publicRaffles.innerHTML = `<div class="list-card empty">Розыгрышей пока нет</div>`;
    return;
  }

  elements.publicRaffles.innerHTML = state.raffles
    .map(
      (raffle) => `
        <button class="public-card${state.currentRaffle?.slug === raffle.slug ? " active" : ""}" type="button" data-slug="${raffle.slug}">
          <strong>${escapeHtml(raffle.title)}</strong>
          <span>${escapeHtml(raffle.prizeText)}</span>
          <small>${raffle.status === "active" ? `Участников: ${raffle.participantCount}` : "Итоги готовы"}</small>
        </button>
      `
    )
    .join("");

  for (const button of elements.publicRaffles.querySelectorAll("[data-slug]")) {
    button.addEventListener("click", async () => {
      const slug = button.dataset.slug || "";
      state.currentRaffleSlug = slug;
      history.replaceState({}, "", `/?raffle=${encodeURIComponent(slug)}`);
      const response = await api(`/api/raffles/slug/${encodeURIComponent(slug)}`);
...
function renderCurrentRaffle() {
  ...
  if (!raffle) {
    elements.raffleTitle.textContent = "Розыгрыш не найден";
    elements.raffleSubtitle.textContent = "Создай новый розыгрыш в админке или открой ссылку из канала.";
  ...
  elements.raffleSubtitle.textContent = `${raffle.prizeText}. Победителей: ${raffle.winnersCount}.`;
  ...
  elements.playerStatus.innerHTML = [
    ...
    statCard("Шанс", `${raffle.chancePercent}%`),
    statCard("Таймер", formatTimeLeft(raffle.endsAt, raffle.status)),
  ].join("");
  ...
  if (!state.auth.telegramId) {
    showError(elements.playerMessage, "Открой mini app внутри Telegram, чтобы участвовать.");
  } else if (!state.auth.initData && state.config.hasTelegramAuth) {
    showError(elements.playerMessage, "Telegram не передал данные авторизации. Открой приложение заново из бота.");
    elements.joinButton.disabled = true;
  } else if (hasJoined) {
    showMessage(elements.playerMessage, `Ты уже в игре. Твой шанс сейчас ${viewerParticipant.chancePercent}%.`);
  } else {
    showMessage(elements.playerMessage, `Вступай в розыгрыш. Текущий шанс для каждого участника: ${raffle.chancePercent}%.`);
  }
...
function showResultMessage(hasJoined, isWinner) {
  if (!hasJoined) {
    showMessage(elements.playerMessage, "Итоги готовы. Ты не участвовал в этом розыгрыше.");
    return;
  }
...
function renderRoulette(raffle, animateToWinner) {
  ...
  const targetIndex = nodes.findIndex((node, index) => index > participants.length * 3 && node.dataset.participantId === winnerId);
...
async function onAdminLogin(event) {
  ...
  try {
    ...
    showError(elements.adminLoginMessage, error.message === "invalid_password" ? "Неверный пароль" : error.message);
  }
}
...
async function joinCurrentRaffle() {
  ...
  try {
    ...
    showMessage(elements.playerMessage, `Ты в игре. Твой шанс сейчас ${response.participant.chancePercent}%.`);
    renderCurrentRaffle();
    renderPublicRaffles();
  } catch (error) {
...
function formatPlayerError(message) {
  switch (message) {
    ...
    case "telegram_init_data_required":
      return "Telegram не передал данные входа. Открой розыгрыш заново из бота.";
    case "telegram_bot_token_required":
      return "На сервере не настроен BOT_TOKEN.";
    case "telegram_init_data_invalid":
      return "Проверка Telegram не пройдена. Открой mini app заново.";
    case "telegram_init_data_expired":
      return "Данные Telegram устарели. Открой ссылку заново.";
    case "raffle_closed":
      return "Розыгрыш уже завершён.";
    default:
      return message;
  }
}
